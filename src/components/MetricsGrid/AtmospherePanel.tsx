...
export function AtmospherePanel({ city }: { city: GeoResult }) {
  ...
  if (isError || !data) return <ErrorMessage message="Atmosphere data unavailable" />

  const { relative_humidity_2m, surface_pressure, visibility } = data.current
  const visibilityKm = (visibility / 1000).toFixed(1)

  const stats = [
    { icon: '💧', label: 'Humidity', value: `${relative_humidity_2m}%` },
    { icon: '🌡️', label: 'Pressure', value: `${Math.round(surface_pressure)} hPa` },
    { icon: '👁️', label: 'Visibility', value: `${visibilityKm} km` },
  ]

  return (
    <div className="bg-[#1e293b] rounded-2xl p-4 shadow-lg">
      <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Atmosphere</h3>
